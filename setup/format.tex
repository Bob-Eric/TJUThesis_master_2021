...
% 这里是为了部分专业同学对于生僻字繁体字的需求写的备用解决方案
% 解决思路：直接使用windows自带的宋体，中易宋体
% 使用方式：（1）将任意一台Windows电脑的C:\Windows\Fonts\simsun.ttc，
%               复制到TJUThesis_master_2021-main/fonts文件夹下
%           （2）对下面两行代码取消注释
%           （3）注释掉下面的 \newcommand{\song}{\songti} 命令
% WH的留言：之所以没有直接将simsun.ttc放入代码中，一方面是文件过大，另一方面是中易宋体并不是免费使用的。
%           不过人家也不会因为你用他们的字体写论文去专门找你。
%           但是有一些其他的宋体是免费的，这就可以请大家自行发挥了。
...
\setCJKfamilyfont{kaigb}[AutoFakeBold = {3.17},Path=fonts/]{楷体_GB2312.ttf}
...
{\kaiGB{}}  % 由于未知的bug，第一次使用kaiGB前面会有空格
...
\titlecontents{chapter}[0em]{\vspace{0pt}\xiaosi\hei\bfseries}%
...
\def\makecover{
  ...
  \begin{titlepage}
    ...
    \begin{center}
      ...
      \includegraphics[height=1.75cm,keepaspectratio]{./preface/tjulogo.png} % 设置校徽 （天津大学关于博士、硕士学位论文统一格式的规定（2021年修订20250605更新扉页版））
      % \vspace*{-0.2cm}
